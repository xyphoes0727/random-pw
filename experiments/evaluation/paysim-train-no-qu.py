"""Paysim streaming evaluation without query-based features.

This script:
- Wraps River anomaly detectors as classifiers.
- Uses an adaptive threshold based on weighted percentiles and EMA.
- Builds a stacking ensemble (HST + One-Class SVM with Logistic meta).
- Streams over Paysim data and logs running metrics.
"""
...
class AnomalyToClassifier:
    ...
    def learn_one(self, x, y=None):
        ...
        Args:
            ...
            y: Optional label; ignored by unsupervised anomaly models.

        Returns:
            The result of the wrapped model's update.
        """
        try:
...
class DynamicThresholdWrapper:
    """Adaptive decision threshold for streaming probabilities.

    Maintains buffers of recent fraud and non-fraud scores, periodically
    computes a weighted percentile, and smooths the threshold via EMA with
    bounds and a grace period.
    ...
    def update(self, y_proba_1, y_true=None):
        """Update internal buffers and return current adaptive threshold.
    ...
    def _compute_weighted_percentile(self):
        """Compute a weighted percentile over fraud and non-fraud buffers.
...
def make_stacking_hst_svm():
    """Create HST + One-Class SVM stacking pipeline with Logistic meta-classifier.
...
class StackingModel:
    ...
    def update_and_metrics(self, data, ground_truth):
        """Perform inference, optional update, and refresh streaming metrics.
