...
class AnomalyToClassifier:
    """Wrapper to adapt a river anomaly
    detector for classifier-like behavior."""
...
class DynamicThresholdWrapper:
    ...
    def update(self, y_proba_1, y_true=None):
        ...
        samples_since_update = (
            self.samples_seen - self.last_percentile_update)
        if samples_since_update >= self.percentile_update_interval:
            self.cached_percentile_threshold = (
                self._compute_weighted_percentile())
    ...
    def _compute_weighted_percentile(self):
        ...
        if not weighted:
            if self.recent_scores:
                return float(np.percentile(np.array(
                    self.recent_scores), self.percentile))
            return 0.5
...
def make_stacking_hst_svm():
    if compose is None:
        raise RuntimeError("river not available")
    base_models = [
...
class StackingModel:
    """Wrapper around a river model that maintains
    adaptive threshold and online metrics."""
    ...
    def __init__(self):
        ...
        self.f1_metric = (
            metrics.F1() if metrics is not None else None)
        self.recall_metric = (
            metrics.Recall() if metrics is not None else None)
        self.auc = (
            metrics.ROCAUC() if metrics is not None else None)
    ...
    def update_and_metrics(self, data, ground_truth):
        """Perform inference, update model
        (if supported), and update metrics."""
        ...
        f1_1 = (
            ...
            if precision_1 and recall_1 and (precision_1 + recall_1) > 0
            else None
        )
        ...
        f1_0 = (
            ...
            if precision_0 and recall_0 and (precision_0 + recall_0) > 0
            else None
        )
        ...
        classification_report = {
            ...
            "overall": {
                ...
                "macro_f1": (
                    ...
                    if f1_0 is not None and f1_1 is not None
                    else None
                ),
            },
        }
        ...
        metrics_snapshot = {
            ...
            "Recall": (
                self.recall_metric.get()
                if self.recall_metric else None
            ),
            "ROC": self.auc.get() if self.auc else None,
            "Precision": (
                self.precision_metric.get()
                if self.precision_metric else None
            ),
            "ConfusionMatrix": {
                "TP": self.tp,
                "FP": self.fp,
