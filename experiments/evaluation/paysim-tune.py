...
def make_stacking_hst_svm(q_hst: float, q_svm: float):
    if compose is None:
...
class StackingModel:
    ...
    def __init__(self, q_hst: float, q_svm: float):

        self.model = make_stacking_hst_svm(q_hst, q_svm)
...
def objective(trial):
    params = {
        "q_hst": trial.suggest_float("q_hst", 0.90, 0.99),
        "q_svm": trial.suggest_float("q_svm", 0.90, 0.99),
    }
    ...
    for idx, row in df.iterrows():
        ...
        if (idx+1 == dfLen):
            finalResult = metrics_snapshot

    print(f"Final Result: {finalResult}")
    return finalResult["F1"]
